...
const fetchProducts = async () => {
  if (cachedProducts) return cachedProducts; // Return cached data if available

  try {
    const { data, error } = await supabase.from('master_dawabag').select('*');
    ...
    cachedProducts = data.map(product => ({
      ...
      price: product.product_selling_user,
      shortDesc: product.generic_name,
      description: product.product_description,
      reviews: product.reviews || [],
      avgRating: product.avgRating,
      offer: product.coupon_applicable
    }));
...
const fetchAds = async () => {
  if (cachedAds) return cachedAds; // Return cached data if available

  try {
    const { data, error } = await supabase.from('master_ads').select('*');
    if (error) throw error;

    cachedAds = data.map(ad => ({
      id: ad.id,
      title: ad.title,
      desc: ad.description,
      cover: ad.image,
      // Add any other fields that are present in your master_ads table
    }));
